...
(function () {
    ...
    function initializeEvents() {
        ...
        $minimizeContainer.mouseenter(function () {
            $minimizeContainer.addClass('collapseHovered')
        });
        $minimizeContainer.mouseleave(function () {
            $minimizeContainer.removeClass('collapseHovered')
        });
        $maximizeContainer = $('#maximizePanelContainer');
        $maximizeContainer.mouseenter(function () {
            if (!MOBILE_DEVICE) $minimizeContainer.addClass('expandHovered')
        });
        $maximizeContainer.mouseleave(function () {
            if (!MOBILE_DEVICE) $minimizeContainer.removeClass('expandHovered')
        });
        ...
        $('#mHideSidebar').on($axure.eventNames.mouseDownName, function (e) {
            startM(e);
        });
        ...
        $(window).on("orientationchange", function () {
            ...
            if (IOS && isMobileMode()) setTimeout(function () {
    ...
    var wasMobile = false;
    var isMobileMode = $axure.player.isMobileMode = function () {
        return isShareApp() || (MOBILE_DEVICE && $(window).width() < 420);
    }
    ...
    function setAdaptiveView() {
        if (typeof noViewport == 'undefined') {
            ...
            if (!isViewOverridden() && !isAnimating) $axure.messageCenter.postMessage('setAdaptiveViewForSize', {
                ...
                $('.leftPanel').animate({'margin-left': -newWidth + 'px'},
                    {
                        duration: 200, complete: function () {
                            $('.leftPanel').width(0).hide().css({'margin-left': ''});
                        }
                    });
        ...
        if (context === 'inspect' || context === 'all') {
            if (!isMobileMode()) {
                ...
                $('.rightPanel').animate({'margin-right': -newWidth + 'px'},
                    {
                        duration: 200, complete: function () {
                            $('.rightPanel').width(0).hide().css({'margin-right': ''});
                        }
                    });
    ...
    $axure.player.expandFromBar = function (hostId, context, isFinalPluginToRestore) {
        ...
        if (context === 'project') {
            ...
            if (!isMobileMode()) {
                ...
                $('.leftPanel').animate({'margin-left': '0px'}, {
                    duration: 200, complete: function () {
                        $('.leftPanel').css({'margin-left': ''});
                    }
                });
            ...
            if (!isMobileMode()) {
                ...
                $('#' + hostId).show();
                $('.rightPanel').animate({'margin-right': '0px'}, {
                    duration: 200, complete: function () {
                        $('.rightPanel').css({'margin-right': ''});
                    }
                });
    ...
    $axure.player.refreshViewPort = function () {
        ...
        $axure.messageCenter.postMessage('setDeviceMode', {
            device: isDevice,
            width: w,
            scaleToWidth: (scaleVal == "1")
        });
    ...
    function isSafari() {
        ...
        var liveSafari = /constructor/i.test(window.HTMLElement) || (function (p) {
    ...
    */
    ...
    function appendProjectOptions() {
        ...
        $(('#projectOptionsHost')).click(function (e) {
            e.stopPropagation();
        });
    ...
    function appendMobileBrowserControlFrame() {
        ...
        if (IOS) $('#mobileControlFrameContainer').on($axure.eventNames.mouseDownName, function (e) {
            e.stopPropagation();
        });
    }
    ...
    function addAppButtonClickListener(id) {
        var func = function () {
            IOS ? window.webkit.messageHandlers.prototypeMenuButtonClick.postMessage(id) : ShareApp.PrototypeMenuButtonClick(id);
        };
    ...
    var isShareApp = function () {
        return /ShareApp/.test(navigator.userAgent);
    }
    ...
    function setUpController() {
        ...
        _player.createPluginHost = function (settings) {
            ...
            if (typeof PREVIEW_INFO === 'undefined') {
                ...
                if (settings.id == 'debugHost') {
                    return false;
                }
                if (settings.id == 'handoffHost') {
                    $('#handoffControlFrameHeaderContainer').show();
                }
            } else {
                // Preview Mode
                if (settings.id == 'handoffHost') {
        ...
        _player.pluginVisibleChanged = function (hostId, visible) {
            if ($axure.player.isCloud && plugins[hostId]) {
                $axure.messageCenter.postMessage('pluginVisibleChanged', {
                    id: hostId,
                    gid: plugins[hostId].gid,
                    visible: visible
                });
            }
        }
        ...
        var interfaceControlHeaderButton_click = function (id) {
            ...
            if (currentVisibleHostId[context] == id) {
                ...
                if (id == "sitemapHost") {
                    $('#sitemapControlFrameContainer').removeClass('selected');
                }
                ...
                if (id == "sitemapHost") {
                    $('#sitemapControlFrameContainer').addClass('selected');
                }
    ...
    $axure.player.addDeviceFraming = function (project, isEdit) {
        ...
        if (!$axure.player.settings.isExpo || project.Platform.Device === 12) {
            return;
        }
