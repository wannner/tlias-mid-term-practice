...
//        var y = JSON.stringify(axEventObject);
        ...
        if (!skipShowDescriptions || eventDescription == 'OnPageLoad') postMessage('axEvent', {
            'label': axObjLabel,
            'type': axObjType,
            'event': axEventObject
        });
            ...
            for (var i = 0; i < axEventObject.cases.length; i++) {
                ...
                $link.click(function (j) {
                        return function () {
                            ...
                            postMessage('axCase', {
                                'item': currentCase.description,
                                'description': currentCase.conditionString,
                                'color': currentCase.caseColorHex
                            })
                                ...
                                for (var target in currentAction.actionInfoDescriptions) {
                                    ...
                                    for (var shortDescription in infoDescriptions) {
                                        postMessage('axInfo', {
                                            ...
                                            'longDescription': infoDescriptions[shortDescription]
                                        });
                                    }
                                }
                            }
    ...
    var _initializeObjectEvents = function (query, refreshType) {
        query.each(function (dObj, elementId) {
            ...
            if (isDynamicPanel) {
                $element.children().scroll(function () {
                    _setCanClick(false);
                });
            }
            ...
            if ($ax.public.fn.IsTreeNodeObject(dObj.type)) {
                ...
                $element.click(function () {
                    $ax.tree.SelectTreeNode(elementId, true);
                });
            }
    ...
    var _initialize = function () {
        ...
        _initializeObjectEvents($ax(function (obj, elementId) {
            return !$ax.repeater.getItemIdFromElementId(elementId);
        }));
    ...
    var _pageLoad = function () {
        ...
        if (!$ax.features.supports.mobile) {
            ...
            $win.bind($ax.features.eventNames.mouseUpName, function (e) {
                _updateMouseLocation(e, true);
            });
        }
