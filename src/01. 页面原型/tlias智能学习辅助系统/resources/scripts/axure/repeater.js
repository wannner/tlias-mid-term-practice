...
$axure.internal(function ($ax) {
    ...
    var _getDataFromDataSet = function (eventInfo, repeaterId, itemId, propName, type) {
        ...
        switch (type) {
            case 'data':
                return data.type == 'text' ? data.text : data
            case 'img':
                return (data.img && data.img[$ax.adaptive.getSketchKey()]) || data.text;
            default:
...
$axure.internal(function ($ax) {
    ...
    var _updateMobileScroll = _dynamicPanelManager.updateMobileScroll = function (panelId, stateId, blockResetScroll) {
        ...
        if (obj.isExpo) {
            ...
            $ax.adaptive.addNiceScroll(stateQuery, {
                touchbehavior: true,
                bouncescroll: false,
                grabcursorenabled: false,
                railmargin: {top: headerHeight, bottom: footerHeight},
                scrollbarid: stateId + "-sb"
            });
            stateQuery.find('.nicescroll-rails').css('margin-top', headerHeight + 'px');
        } else {
            $ax.adaptive.addNiceScroll(stateQuery, {
                emulatetouch: true,
                horizrailenabled: obj.scrollbars != 'verticalAsNeeded'
            }, blockResetScroll);
        }
        ...
        stateChildrenQuery.children('.panel_state_content').each(
            function () {
                $(this).children('.ax_dynamic_panel').each(
                    function () {
                        _updatePanelPercentWidth(this.id);
                    }
                );
            }
        );
    ...
    var _getShownStateObj = function (id) {
        return $ax('#' + _getShownStateId(id));
    }

    _dynamicPanelManager.getShownState = function (id) {
        return $jobj(_getShownStateId(id));
    };
    ...
    var _compressChildrenHelper = function (id, children, vert, threshold, delta, clamp, easing, duration, parentLayer) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            if ($ax.getTypeFromElementId(childId) == $ax.constants.LAYER_TYPE) {
                ...
                var layerClamp = {
                    prop: clamp.prop,
                    offset: clamp.offset,
                    start: clamp.start + clampProp,
                    end: clamp.end + clampProp
                };
        ...
        if (allMove && parentLayer) {
            ...
            for (var i = 0; i < toMove.length; i++) {
                $ax('#' + toMove[i]).moveBy(vert ? 0 : delta, vert ? delta : 0, easing == 'none' ? {} : {
                    duration: duration,
                    easing: easing
                });
            }
